...
the solution with $t\ge 0$. Since $g$ is continuous with $g|_{S^{n-1}} = \id_{S^{n-1}}$, the 
theorem follows from the lemma below.
\end{proof}

\begin{lemma}\label{lemma:7-2}
  There is no continuous map $g:D^n\to S^{n-1}$, with $g|_{S^{n-1}} = \id_{S^{n-1}}$.
\end{lemma}
...
\begin{proof}
  ...
  Theorem \ref{theorem:6-13} is 1-dimensional. On the other hand Lemma \ref{lemma:6-14} evaluates $f_i^*$ to
  be multiplication with $(-1)^{n+1}$. Hence $n$ is odd.
  Conversely, for $n=2m-1$, we can define a vector field $v$ with 
  \begin{align*}
    v(x_1,x_2,\ldots,x_{2m}) = (-x_2, x_1, -x_4, x_3, \ldots, -x_{2m}, x_{2m-1}).
  \end{align*}
\end{proof}
...
(Tangent vector fields $v_1,\ldots,v_d$ on $S^n$ are called linearly independent if for every
$s\in S^n$ the vectors $v_1(x),\ldots,v_d(x)$ are linearly independent.)

\vskip1.5em
\textbf{Adams' theorem}\label{adams-theorem}
For $n=2m-1$, let $2m=(2c+1)2^{4a+b}$, where $0\le b\le 3$. The maximal number of 
linearly independent tangent vector field on $S^n$ is equal to $2^b+8a-1$.
...
\begin{proof}
  ...
  \begin{align*}
    U_p = \left\{x\in\RR^n\mid d(x, p)< \frac12d(p, A)\right\}
  \end{align*}
...
\begin{proof}
  We merely have to compose the homeomorphism $h$ from Lemma \ref{lemma:7-6} with
...
$\RR^3-\Sigma$ is not homeomorphism to $\RR^3-S^2$. This and numerous examples are treated in [Rushing].
...
\begin{proof}
  ...
  components (the open half-spaces), while $\RR^{n+1} - B$ is connected. Hence this case 
  cannot occur.
\end{proof}
...
\begin{proof}
  ...
  are 
  \begin{align*}
    D^n = \{x\in \RR^n \mid \|x\|<1 \} \text{ and } W = \{x\in\RR^n\mid \|x\|>1\}
  \end{align*}
  By choosing $r=\max\limits_{x\in\Sigma}\|x\|$, the connected set 
  \begin{align*}
    rW = \{x\in\RR^n \mid \|x\|>r\}
  \end{align*}
...
\begin{align*}
  D = \{x\in\RR^n \mid \|x-x_0\|\le \delta\}
\end{align*}
...
\begin{proof}
  ...
  and apply $H^{n - 1}$ to get the commutative diagram
  \begin{center}
    \begin{tikzcd}
...
\begin{remark}\label{remark:7-17}
  The above result about $H^*(U_1)$ might suggest that $U_1$ is contractible
...
\begin{example}\label{example:7-18}
  ...
  \begin{align*}
    V = \RR^n - \biggl(\bigcup_{j=1}^m K_j\biggr).
  \end{align*}
  ...
  \begin{align*}
    V_1 = \RR^n - \biggl(\bigcup_{j=1}^{m-1} K_j\biggr).
  \end{align*}
