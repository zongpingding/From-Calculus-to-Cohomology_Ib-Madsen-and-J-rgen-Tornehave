...
\begin{definition}
  ...
  the line segemtn $\{tx_0+(1-t)x\mid t\in[0,1]\}$ is contained in $X$ for all $x\in X$.
\end{definition}
...
\begin{align*}
  \ker(\rot)  & = \text{Kernel of rot} \\
  \im(\grad)  & = \text{Image of grad}
\end{align*}
...
In analogy with \eqref{eq:1-3} we introduce
...
\begin{example}\label{example:1-7}
  Let $S = \{(x_1, x_2, x_3)\in \RR^3\mid x_1^2 + x_2^2 = 1, x_3 = 0\}$ be the unit circle
