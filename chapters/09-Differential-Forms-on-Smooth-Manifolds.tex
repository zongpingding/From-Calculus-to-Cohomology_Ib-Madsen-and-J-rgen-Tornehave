...
\begin{definition}\label{definition:9-2}
...
Note that the linear isomorphism $\Phi_h$ in Definition \ref{definition:9-2} can be identified with $D_ph$
...
Definition \ref{definition:8-8} implies that $D_pi:T_pM^m\to T_p\RR^l\simee \RR^l$ is injective. In 
...
a basis for $T_pM$
\begin{align*}
  \left(\frac{\partial }{\partial x_1}\right)_p, \ldots,
  \left(\frac{\partial }{\partial x_m}\right)_p,
\end{align*}

where $(\frac{\partial }{\partial x_i})_p$ is the image under $D_{h(p)}h^{-1}:\RR^m\to T_pM$ of the 
$i$-th standard basis vector $e_i = (0, \ldots, 1, \ldots, 0)\in\RR^m$. A tangent vector $X_p\in T_pM$
can be written uniquely as 
\begin{align}\label{eq:9-4}
  X_p = \sum_{i=1}^{m }{\left(\frac{\partial }{\partial x_i }\right)_p}
\end{align}

where $\F{a}=(a_1, \ldots, a_m)\in\RR^m$. If $X_p = [\alpha]$, where $\alpha:I\to U$ with $\alpha(0)=p$ is 
...
\begin{definition}\label{definition:9-5}
...
$\xi_1, \ldots, \xi_{k+1}\in T_pM$. We choose $v_1, \ldots, v_{k+1}\in\RR^n$ so that 
$D_x(g\circ \phi)(v_i) = \xi_i$. We must show that
\begin{align*}
  \dd_yg^*(\omega)(w_1, \ldots, w_{k+1})
  = \dd_x(g\circ \phi)^*(\omega)(v_1, \ldots, v_{k+1})
\end{align*}
...
\begin{definition}\label{definition:9-7}
...
\begin{definition}\label{definition:9-8}\index{orientable manifold}
  \begin{enumerate}[(i)]
...
On the Euclidean space $\RR^n$ we have the orientation form $\dd x_1\wedge\ldots\wedge\dd x_n$, which
represents the standard orientation\index{standard orientation}\index{orientation!standard} of $\RR^n$.

Let $M^n$ be oriented by the orientation form\index{orientation!form} $\omega$. A basis $b_1, \ldots, b_n$ of $T_pM$ is said
to be positively or \Index{negatively oriented} with respect to $\omega$ depending on whether
the number
\begin{align*}
  \omega_p(b_1 ,\ldots, b_n) \in \RR
\end{align*}
...
$b_1, \ldots, b_n$ is a basis of $T_pM$, with $\omega_{i,p}(b_1, \ldots ,b_n) > 0$, then 
$\omega_{i',p}(b_1, \ldots ,b_n)>0$ for every other if with $p\in V_i$, and in the formula
\begin{align*}
  \omega_p(b_1, \ldots ,b_n)
  = \sum_{i}^{}{\phi_i(p)\omega_{i,p}(b_1, \ldots ,b_n)}
\end{align*}

all terms are positive (or zero). Thus $\omega$ is an orientation form on $M$, and $b_1, \ldots, b_n$
...
type, then $\tau=f\cdot \omega$ and $\tau_p(b_1, \ldots, b_n) = f(p)\omega_p(b_1, \ldots , b_n)$. 
Since both $\tau_p$ and wp are positive on $b_1, \ldots, b_n$ we have $f(p) > 0$. Hence $\omega$ 
and $\tau$ detennine the same orientation.\hfill\(\qedsymbol\)

\begin{definition}\label{definition:9-12}
...
\begin{definition}\label{definition:9-15}
...
where $e_1, \ldots, e_n$ is the standard basis of $\RR^n$. These functions are called the
...
two positively oriented orthonormal bases $b_1, \ldots, b_n$ and $b'_1, \ldots, b'_n$ in the 
...
\begin{align}
  \omega_(b'_1, \ldots, b'_n) = (\det C) \omega_p (b_1, \ldots, b_n).
\end{align}

Positivity ensures that det $C > 0$; but then $\det C = 1$. Hence there exists a
function $\rho:M\to (0,\infty)$ such that $\rho(p) = \omega_p(b_1, \ldots, b_n)$ for every 
positively oriented orthonormal basis $b_1, \ldots, b_n$ of $T_pM$. We must show that $\rho$ is 
...
\begin{align}\label{eq:9-13}
  b_i(q) = \sum_{j=1 }^{n}{ a_{ij}(q)X_j(q) }, \qquad i=1, \ldots, n.
\end{align}
...
\begin{align}\label{eq:9-14}
  \begin{aligned}
    \rho\circ f(q) 
      & = \omega_{f(q)}(b_1(1), \ldots, b_n(q))
        = (\det A(q))\omega_{f(q)}(X_1(q), \ldots, X_n(q))\\
      & = (\det A(q))(f^*\omega)_q (e_1, \ldots, e_n).
  \end{aligned}
\end{align}
...
\begin{addendum}\label{addendum:9-17}
  ...
  \begin{align}\label{eq:9-15}
    f^*(\R{vol}_M) = \sqrt{\det (g_{ij})} \dd x_1\wedge\ldots\wedge\dd x_n.
  \end{align}
\end{addendum}
...
\begin{align}\label{eq:9-16}
  f^*(\R{vol}_M) = (\det A(x))^{-1}\dd x_1\wedge\ldots\wedge\dd x_n.
\end{align}
...
\begin{example}\label{example:9-18}
  ...
  \begin{align}\label{eq:9-17}
    \omega_{0x}(w_1, \ldots, w_{n-1})
    = \det (x, w_1, \ldots, w_{n-1})\in \alt^{n-1}(\RR^n) 
  \end{align}
  for $x\in\RR$. Since $\omega_{0x}(e_1, \ldots, \hat{e}_i, \ldots, e_n) = (-1)^{i-1}x_i$, we have 
  \begin{align}\label{eq:9-18}
    \omega_0 = \sum_{i=1}^{n }{(-1)^{i-1} x_i \dd x_1\wedge\ldots\wedge\dd x_n}.
  \end{align}

  If $x\in S^{n-1}$ and $w_1, \ldots, w_{n-1}$ is a basis of $T_xS^{n-1}$ then $w, w_1, \ldots, w_{n-1}$
  ...
  basis $w_1, \ldots, w_{n-1}$ of $T_xS^{n-1}$ is positively oriented if and only if the basis
  $w, w_1, \ldots, w_{n-1}$ for $\RR^n$ is positively oriented.

  We give $S^{n-1}$ the Riemannian structure induced by $\RR^n$. Then \eqref{eq:9-17} implies that
  ...
  \begin{align*}
      \omega_x(v_1, \ldots, v_{n-1}) 
        & = \omega_{0r(x)}(D_xr(v_1), \ldots, D_xr(v_{n-1})) \\
        & = \|x\|^{-1}\det (x, D_x r(v_1), \ldots, D_xr(v_{n-1})).
  \end{align*}
  ...
  \begin{align*}
    \omega_x (v_1, \ldots, v_{n-1})
    & = \|x\|^{-n} \det (x, w_1, \ldots, w_{n-1}) 
      = \|x\|^{-n} \det (x, v_1, \ldots, v_{n-1}) \\
    & = \|x\|^{-n} \omega_{0x}(v_1, \ldots, v_{n-1}).
  \end{align*}

  Hence the closed form $\omega$ is given by 
  \begin{align}\label{eq:9-19}
    \omega = \frac{1}{\|x\|^n} \sum_{i=1}^{n }{
      (-1)^{i-1}x_i \dd x_1\wedge\ldots\wedge\widehat{\dd x_i}\wedge\dd x_n
    }
  \end{align}
\end{example}
...
$Y_1(p), \ldots, Y_k(p)$ form an orthonormal basis of $T_pM^\perp$ for every $p\in W$.
\end{lemma}

\begin{proof}
  On a coordinate patch around $p_0\in M$, there exist smooth tangent vector
fields $X_1, \ldots, X_n$, which at every point $p$ yield a basis of $T_pM$, cf. 
Remark \ref{remark:9-4}. Choose a basis $V_1, \ldots, V_k$ of $T_{p_0}M^\perp$. Since 
the $(n+k)\times (n+k)$ determinant 
\begin{align*}
  \det(X_1(p), \ldots, X_n(p), V_1, \ldots, V_k) 
\end{align*}
...
\begin{align*}
  X_1(p), \ldots, X_n(p), V_1, \ldots, V_k\qquad (p\in W)
\end{align*}
of $\RR^{n+k}$ gives an orthonormal basis 
\begin{align*}
  \tilde{X}_1(p), \ldots,\tilde{X}_n(p), Y_1(p), \ldots, Y_k(p),
\end{align*}

where the first $n$ vectors span $T_pM$. The formulas of the Gram-Schmidt orthonormalization 
show that all $X_i$ and $Y_j$ are smooth on $W$, so that $Y_1, \ldots, Y_k$ have the desired 
properties.
\end{proof}
...
\begin{proposition}\label{prop:9-22}
  ...
  \begin{enumerate}[(i)]
    \item There is a 1-1 correspondence between smooth normal vector fields $Y$ on
      ...
      given by
      \begin{align*}
        \omega_p(W_1, \ldots, W_n) = \det (Y(p), W_1, \ldots, W_n)
      \end{align*}
      for $p\in M, W_i\in T_pM$.
    \item This induces a 1-1 correspondence between Gauss maps $Y\colon M\to S^n$ and
      orientations of $M$.
  \end{enumerate}
\end{proposition}
...
\begin{theorem}[Tubular neighborhoods]\label{theorem:9-23}
  ...
  \begin{enumerate}[(i)]
    ...
    \item If $\epsilon:M\to\RR$ is smooth and $0<\epsilon(p)<\rho(p)$ for all $p\in M$ then 
      \begin{align*}
        S_\epsilon = \{x\in V \mid \|x-r(x)\| = \epsilon(r(x)) \}
      \end{align*}
...
vector fields $Y_1, \ldots,Y_k$ as in Lemma \ref{lemma:9-21}, defined on an open neighborhood $W$
...
\begin{align*}
  \frac{\partial f }{\partial x_1}(0), \ldots, 
  \frac{\partial f }{\partial x_n }(0),
  Y_1(p_0), \ldots, Y_k(p_0)
\end{align*}
...
By Definition \ref{definition:8-8} there exists an open neighborhood $\hat{W}$ of $p_0$ 
...
\begin{align*}
  V_0' = \{x\in V_0\mid r_0(x)\in\R{int}K_1 \text{ and } \|x-r_0(x)\|<\epsilon\},
\end{align*}
...
\begin{align*}
  S = \{(\rho, t)\in W_0\times \RR^k \mid \|t\|=\epsilon(p)<\epsilon_0\}.
\end{align*}

The projection of $S$ on $W_0$ is the open set 
\begin{align*}
  U = \{p\in W_0\mid \epsilon(p)<\epsilon_0\}\sseq M.
\end{align*}
...
\begin{align*}
  N_\epsilon = \{x\in V\mid \|x-r(x)\|\le \epsilon\}.
\end{align*}
...
\begin{proof}
  ...
  finitely many open balls $U_1, \ldots, U_r$ in $\RR^{n+k}$ such that their union $U=U_1\cup\ldots\cup U_r$
