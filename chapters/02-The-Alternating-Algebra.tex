...
\begin{align*}
  f: \underbrace{V\times V\times \ldots \times V}_{k\text{ times}} \to \RR
\end{align*}
...
\begin{definition}\index{alternating!map}
  ...
  $\omega(\xi_1, \ldots, \xi_k) = 0$ whenever $\xi_i=\xi_j$ for some pair $i\neq j$. The vector space
  of alternating, $k$-linear maps is denoted by $\alt^k(V)$\index{$\alt^k(V)$}.
\end{definition}


We immediately note that $\alt^k(V) = 0$ if $k > \dim V$. Indeed, let $e_1, \ldots, e_n$ be a
...
with $\lambda_J = \lambda_{j_1, 1}, \ldots, \lambda_{j_k, k}$. Since $k>n$, there must be at least one repetition
among the elments $e_{j_1}, \ldots, e_{j_k}$. Hence $\omega(e_{j_1}, \ldots, e_{j_k}) = 0$.

The symmetric group of permutations of the set $\{1, \ldots,k\}$ is denoted by $S(k)$.
...
\begin{example}\label{example:2-3}
  Let $V = \RR^k$ and $\xi_i = (\xi_{i1}, \ldots, \xi_{ik})$. The function $\omega(\xi_1, \ldots, \xi_k) = \det((\xi_{ij}))$
...
\begin{definition}\label{definition:2-4}
  A \Index{$(p, q)$-shuffle} $\sigma$ is a permutation of the set $\{1, \ldots, p + q\}$ satisfying
  \begin{align*}
  ...
  set $\{\sigma(1),\ldots,\sigma(p)\}$, the cardinality of $S(p, q)$ is $\binom{p + q}{p}$.
\end{definition}


\begin{definition}[Exterior product]\label{definition:2-5}
...
\begin{proof}
  We first show that $\omega_1\wedge\omega_2(\xi_1,, \xi_2, \ldots, \xi_{p+q}) = 0$ when $\xi_i = \xi_j$.
  We let
  \begin{enumerate}[(i)]
  ...
  If $\sigma\in S_0$ then either $\omega_1(\xi_{\sigma(1)}, \ldots, \xi_{\sigma(p)}) = 0$
  or $\omega_2(\xi_{\sigma(p+1)}, \ldots, \xi_{\sigma(p+q)}) = 0$, since $\xi_{\sigma(1)} = \xi_{\sigma(2)}$
...
\begin{lemma}\label{lemma:2-7}
  A $k$-linear map $\omega$ is alternating if $\omega(\xi_1, \ldots, \xi_k) = 0$ for all $k$-tuples
...
\begin{proof}
  ...
  \begin{align*}
    \omega(\xi_1, \ldots, \xi_i, \xi_{i+1}, \ldots, \xi_k)
    = - \omega(\xi_1, \ldots, \xi_{i+1}, \xi_i, \ldots, \xi_k).
  \end{align*}
...
\begin{proof}
  ...
  Note that
  \begin{align*}
    \omega_2(\xi_{\sigma\gamma(1)}, \ldots, \xi_{\sigma\gamma(q)})
    & = \omega_2(\xi_{\tau\sigma(p+1)}, \ldots, \xi_{\tau\sigma(p+q)}). \\
    \omega_1(\xi_{\sigma\gamma(q+1)}, \ldots, \xi_{\sigma\gamma(p+q)})
    & = \omega_1(\xi_{\tau\sigma(1)}, \ldots, \xi_{\tau\sigma(p)}).
  \end{align*}
...
\begin{proof}
  ...
  We will need the subset $S(\bar{p}, q, r)$ and $S(p, q, \bar{r})$ of $S(p, q, r)$ given by
  \begin{align*}
  \sigma \in S(\bar{p}, q, r) \Longleftrightarrow \sigma \text{ is the identity on }  & \{1, \ldots, p\} \text{ and } \sigma\in S(p, q, r) \\
  \sigma \in S(p, q, \bar{r}) \Longleftrightarrow \sigma \text{ is the identity on }  & \{p+q+1, \ldots, p+q+r\}                           \\
  ...
  \begin{align}\label{eq:2-2}
    \begin{aligned}
      ...
      S(p, q, r) \times S(p, q, \bar{r}) & \xra[\cong] S(p, q, r); \quad (\sigma, \tau)\mapsto\tau\circ\sigma.
    \end{aligned}
  \end{align}
  ...
  $[(\omega_1\wedge\omega_2)\wedge\omega_3](\xi_1, \ldots, \xi_{p+q+r})$, employing the second equation in \eqref{eq:2-2}.
\end{proof}
...
Note that in this formula $\{\sigma(1), \ldots,\sigma(p)\}$ and $\{\sigma(p + 1), \ldots, \sigma(p + q)\}$ are not
...
\begin{definition}\label{definition:2-11}
  \begin{enumerate}[(i)]
    \item A graded $\RR$-algebra $A_*$ is a sequence of vector spaces $A_k, k = 0,1, \ldots$,
...
The product from Definition \ref{definition:2-5} is a bilinear map from $\alt^p(V)\times\alt^q(V)$ to 
...
\begin{lemma}\label{lemma:2-13}
  For 1-forms $\omega_1, \ldots, \omega_p\in\alt^1(V)$, we have 
  \begin{align*}
...
\begin{proof}
  The case $p=2$ is abvious. We procced by induction on $p$. According to Definition \ref{definition:2-5},
  \begin{align*}
  ...
  where $(\xi_1, \ldots, \hat{\xi_j}, \ldots, \xi_p)$ denotes the $p-1$-tuple where 
  $\xi_j$ has been omitted. The lemma follows by expanding the determinant by the first row.
\end{proof}


Note, from Lemma \ref{lemma:2-13}, that if the 1-forms $\omega_1, \ldots, \omega_p\in\alt^1(V)$ 
are linearly independent then $\omega_1\wedge\ldots\wedge\omega_p\neq 0$. Indeed, we can choose 
elements $\xi\in V$ with $\omega_i(\xi_i) = 0$ for $i\neq j$ and $\omega_j(\xi_j) = 0$, so that 
$\det(\omega_i(\xi_j)) = 1$. Conversely, if $\omega_1, \ldots, \omega_p$ are linearly dependent,
we can express one of them, say $\omega_p$, as a linear combination of the others. If 
$\omega_p = \sum_{i=1}^{p-1}{r_i\omega_i}$, then
\begin{align*}
  \omega_{1}\wedge\ldots\wedge\omega_{p-1}\wedge\omega_{p}
  = \sum_{i=1}^{p-1}r_{i}\omega_{1}\wedge\ldots\wedge\omega_{p-1}\wedge\omega_{i}=0,
\end{align*}
...
\begin{lemma}
  For 1-form $\omega_1, \ldots, \omega_p$ on $V$, $\omega_1\wedge\ldots\wedge\omega_p \neq 0$ if and only if
  they are linearly independent.
\end{lemma}

\begin{theorem}\label{theorem:2-15}
Let $e_1, \ldots, e_n$ be a basis of $V$ and $\epsilon_1, \ldots, \epsilon_n$ the dual basis of $V^*$. Then 
\begin{align*}
  \left\{\epsilon_{\sigma(1)}\wedge\ldots\wedge\epsilon_{\sigma(k)}\right\}_{\sigma\in S(p, n-p)}
\end{align*}
...
\begin{proof}
  Since $\epsilon_i(e_j) = 0$ when $i\neq j$, and $\epsilon_i(e_i)= 1$, Lemma \ref{lemma:2-13} gives 
  \begin{align}\label{eq:2-3}
    \epsilon_{i_1}\wedge\ldots\wedge\epsilon_{i_p}(e_{j_1},\ldots,e_{j_p})
    = \left\{\begin{aligned}
      & 0             && \text{ if } \{i_1, \ldots, i_p\}\neq \{j_1, \ldots, j_p\},\\
      & \sign(\sigma) && \text{ if } \{i_1, \ldots, i_p\} = \{j_{\sigma(1)}, \ldots, j_{\sigma(p)}\}.
    \end{aligned}\right.
  \end{align}
  ...
  for any alternating $p$-form. Thus $\epsilon_{\sigma(1)}\wedge\ldots\wedge\epsilon_{\sigma(p)}$ generates 
  ...
  evaluated on $(e_{\sigma(1)}, \ldots, e_{\sigma(p)})$ gives $\lambda_{\sigma} = 0$.
\end{proof}

Note from Theorem \ref{theorem:2-15} that $\alt^n(V)\xra[\cong]\RR$ if $n = \dim V$ and, as mentioned
earlier, that $\alt^p(V) = 0$ if $p > n$. A basis of $\alt^n(V)$ is given by $\epsilon_{1}\wedge\ldots\wedge\epsilon_{n}$.
...
by setting $\alt^p(f)(W)(\xi_1, \ldots, \xi_p) = \omega(f(\xi_1), \ldots, f(\xi_p))$. For the composition of
...
\ref{theorem:2-16} below it follows that $\dd = \det(f)$. We shall also be using other maps 
% ===> WRONG theorem number for Theorem 2.16 <===
\begin{align*}
...
\begin{proof}
Choose a basis $e_1, \ldots, e_n$ of $V$ Assume first that $e_1, \ldots, e_n$ are eigenvectors of $f$,
\begin{align*}
  f(e_i) = \lambda_i e_i, i = 1, \ldots, n.
\end{align*}

Let $\epsilon_1, \ldots, \epsilon_n$ be the dual basis of $\alt^1(V)$. Then
\begin{align*}
  \mathrm{Alt}^p(f)\big(\epsilon_{\sigma(1)}\wedge\ldots\wedge\epsilon_{\sigma(p)}\big)
  = \lambda_{\sigma(1)}\cdot\ldots\lambda_{\sigma(p)}\epsilon_{\sigma(1)}\wedge\ldots\wedge\epsilon_{\sigma(p)}
\end{align*}

and 
\begin{align*}
  \tr \alt^p(f) = \sum_{\sigma\in S(p, n-p)}^{}{\lambda_{\sigma(1)}\cdot\ldots\lambda_{\sigma(p)}}.
\end{align*}
...
\begin{align*}
  ...
  = \sum{(-1)^{n-p}\left(\sum{\lambda_{\sigma(1)}\cdot\ldots\lambda_{\sigma(p)}}\right)t^{n-p}}.
\end{align*}

This proves the formula when $f$ is diagonal.

If $f$ is replaced by $gfg^{-1}$, with $g$ an isomorphism on $V$, then both sides of the
...
by the functor property. Hence $\tr\alt^p(g\circ f\circ g^{-1}) = \tr\alt^p(f)$. Consider the
set
\begin{align*}
  D = \{gf^{-1}g^{-1} \mid f \text{ diagonal }, g \in GL(V)\}.
\end{align*}
