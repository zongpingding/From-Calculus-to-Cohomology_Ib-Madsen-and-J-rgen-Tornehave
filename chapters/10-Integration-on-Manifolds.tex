...
\begin{align*}
  \omega = f(x)\dd x_1\wedge\ldots\wedge\dd x_n,
\end{align*}

where $f\in C^\infty(\RR^n, \RR)$ has compact support. We then define 
\begin{align*}
  \int_{\RR^n}^{}{f(x)\dd x_1\wedge\ldots\wedge\dd x_n }
...
\begin{proof}
  ...
  \begin{align*}
    \phi^*(\omega)
    & = f(\phi(x))\det(D_x\phi)\dd x_1\wedge\ldots\wedge\dd x_n\\ 
    & = \delta f(\phi(x))|\det(D_x\phi)|\dd x_1\wedge\ldots\wedge\dd x_n.
  \end{align*}
...
\begin{remark}\label{remark:10-4}
  ...
  \end{align*}
  where $\phi$ is determined by $(h^{-1})^*(\sigma) = \phi(x)\dd x_1\wedge\ldots\wedge\dd x_n$. Since 
...
\begin{definition}\label{definition:10-5}
...
\end{align}
where $\RR^n_- = \{(x_1, \ldots x_n)\in\RR^n\mid x_1\le 0\}$. 
\end{definition}
...
The Jacobi matrix at the point $q=h(p)\in\partial\RR^n_-$ for $\phi=(\phi_1, \ldots, \phi_n)$ 
has the form 
\begin{align*}
...
\begin{lemma}\label{lemma:10-6}
  ...
  vector then a basis $v_2,\ldots.v_n$ for $T_p\partial N$ is positively oriented if and only if 
  the basis $v_1,v_2,\ldots,v_n$ for $T_pM$ is positively oriented.
\end{lemma}
...
\begin{remark}\label{remark:10-7} 
  \begin{enumerate}[(i)]
    ...
    \item In the case $n = 1$, Lemma \ref{lemma:10-6} holds in the following modified form. An
      orientation of $\partial N$ consists of a choice of $\R{sign}$, $+$ or $-$, for every point
...
\begin{proof}
  ...
  Choose a smooth atlas on $M$ consisting of charts of the type of Definition \ref{definition:10-5}
...
\begin{example}\label{example:10-11}
  ...
  \begin{align*}
    \omega = \frac{1}{\|x\|^n} \sum_{i=1}^{n }{(-1)^{i-1}x_i\dd x_1\wedge\ldots\wedge\widehat{\dd x_i}\wedge\ldots\wedge\dd x_n}.
  \end{align*}
...
\begin{example}\label{example:10-12}
  ...
  \begin{align*}
    \omega_0 = \sum_{i=1}^{n}{(-1)^{i-1}x_i \dd x_1\wedge\ldots\wedge\widehat{\dd x_i}\wedge\ldots\wedge\dd x_n}.
  \end{align*}
  Since $\omega_{0|S^{n-1}} = \R{vol}_{S^{n-1}}$ and $\dd\omega_0 = n\dd x_1\wedge\ldots\wedge\dd x_n$ we have that 
  \begin{align*}
    \R{Vol}(S^{n-1}) 
    = \int_{S^{n-1}}\omega_0
    = \int_{D^n}\dd\omega_0
...
\begin{proof}
  ...
  that $\kappa\in\Omega^{n-1}_c(\RR^n)$, such that $\dd\kappa=\omega$. We can write $\omega=f(\FF{x})\dd x_1 \wedge\ldots\wedge\dd x_n$,
  and let 
  \begin{align*}
    \kappa = \sum_{j=1}^{n }{(-1)^{i-1}f_j(\FF{x})\dd x_1\wedge\ldots\wedge\widehat{\dd x_j}\wedge\ldots\wedge\dd x_n}. 
  \end{align*}
  A simple calculation gives 
  \begin{align*}
    \dd\kappa = \biggl(\sum_{j=1}^{n}{\frac{\partial f_j}{\partial x_j}}\biggr)\dd x_1\wedge\ldots\wedge\dd x_n.
  \end{align*}
  ...
  $f_1, \ldots, f_n$ in $C_c^\infty(\RR^n)$ such that
  \begin{align*}
  ...
  \begin{align}\label{eq:10-9}
    g(x_1, \ldots, x_{n-1}) = \int_{-\infty}^{\infty} f(x_1, \ldots, x_{n-1}, x_n)\dd x_n.
  \end{align}
  ...
  $g_1, \ldots, g_{n-1}$ in $C_c^\infty(\RR^{n-1}, \RR)$ with 
  ...
  \begin{align}\label{eq:10-11}
    f_j(x_1, \ldots, x_n) = g_j(x_1, \ldots, x_{n-1})\rho(x_n), 1\le j\le n-1.
  \end{align}
  ...
  \begin{align}\label{eq:10-13}
    f_n(x_1, \ldots, x_{n-1}, x_n) = \int_{-\infty}^{x_n} h(x_1, \ldots, x_{n-1}, t)\dd t.
  \end{align}
  ...
  limit $x_n$ is replaced by $\infty$. Now \eqref{eq:10-10}, \eqref{eq:10-11} and \eqref{eq:10-12} yield 
  that
  \begin{align*}
    h(x_1, \ldots, x_{n-1}, x_n) 
    & = f(x_1, \ldots, x_{n-1}, t) - \sum_{j=1}^{n-1}{\frac{\partial g_j}{\partial x_j}}\rho(t)\\
    & = f(x_1, \ldots, x_{n-1}, t) - g(x_1, \ldots, x_{n-1})\rho(t).
  \end{align*}
  Finally from \eqref{eq:10-10} it follows that 
  \begin{align*}
    & \int_{-\infty}^\infty h(x_1, \ldots, x_{n-1}, t)\dd t \\
    = & \int_{-\infty}^\infty f(x_1, \ldots, x_{n-1}, t)\dd t 
      - g(x_1, \ldots, x_{n-1})\int_{-\infty}^\infty \rho(t)\dd t\\
    =\; & 0.
  \end{align*}
\end{proof}
...
\begin{lemma}\label{lemma:10-16}
  ...
  let $p, q\in M$. There exists indices $\alpha_1, \ldots, \alpha_k$ such that 
  \begin{enumerate}[(i)]
...
a finite sequence of indices $\alpha_1, \ldots, \alpha_k$ from $A$, such that (i) and (ii) are 
...
\begin{proof}
  ...
  $\RR^n$. We apply Lemma \ref{lemma:10-16} to find open sets $U_2, \ldots, U_k$, diffeomorphic 
  to $\RR^n$, such that $U_{i-1}\cap U_i\neq \ns$ for $2\le i\le k$ and $U_k\sseq W$. We use Lemma 
  \ref{lemma:10-17} to successively choose $\kappa_1, \ldots, \kappa_{k-1}$ in $\Omega^{n-1}_c(M)$ such that 
  \begin{align*}
...
Then $\omega -\dd\kappa=\dd\tau$, so $\tau+\kappa$ maps $\omega$ under $\dd$. \hfill\(\qedsymbol\)