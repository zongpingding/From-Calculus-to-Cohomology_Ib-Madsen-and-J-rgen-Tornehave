...
countable system of open sets $V = \{U_i\mid i\in \NN\}$, such that every open set can be
written in the form $\cup_{i\in I}U_i$, where $I\sseq\NN$.

For instance $\RR^n$ has a countable basis for the topology given by
\begin{align*}
  \C{V} = \{\mrg{D}\mid x=(x_1, \ldots, x_n), x_i\in \QQ; \epsilon\in\QQ, \epsilon>0\}
\end{align*}
...
\begin{definition}\label{definition:8-1}
...
\begin{remark}\label{remark:8-2}
  ...
  for $\tan$ and $\arctan$.) Thus in Definition \ref{definition:8-1} it does not matter whether we require
  that $M^n$ is locally homeomorphic to $\RR^n$ or to an open set in $\RR^n$.
\end{remark}


\begin{definition}\label{definition:8-3} 
  \begin{enumerate}[(i)]
    ...
    \item A system $A = \{h_i: U_i\to U_i'\mid i\in J\}$ of charts is called an atlas, provided 
      $\{U_i \mid i\in J\}$ covers $M$.
...
Note in Definition \ref{definition:8-3}.(iii) that $h_i(U_i\cap U_j)$ is open in $\RR^n$.
...
\begin{definition}\label{definition:8-4}
...
\begin{example}\label{example:8-5}
  The $n$-dimensional sphere $S^n = \{x\in\RR^{n+1}\mid \|x\|=1\}$ is an $n$-dimensional smooth 
  manifold. We define an atlas with $2(n+1)$ charts $(U_{\pm i}, h_{\pm i})$ where 
  \begin{align*}
    U_{+i} = \{x\in S^n\mid x_i>0\},
    && U_{-i} = \{x\in S^n\mid x_i<0\}
  \end{align*}

  and $h_{\pm i}:U_{\pm i}\to \mrg{D}^n$ is the map given by $h_{\pm i}(x) = (x_1, \ldots, \hat{x}_i, \ldots, x_{n+1})$. 
  The circumflex over $x_i$ denotes that $x_i$ is omitted. The inverse map is 
  \begin{align*}
    h_{\pm i}^{-1}(u) = \left(u_1, \ldots, u_{i-1}, \pm\sqrt{1-\|u\|^2}, u_i, \ldots, u_n\right)
  \end{align*}
...
\begin{example}[The projective space $\RR\B{P}^n$]\label{example:8-6}\index{projective space}
  ...
  \end{align*}
  $i=1, \ldots, n$. This gives a smooth atlas on $\RR\B{P}^n$.
\end{example}

\begin{definition}\label{definition:8-7}
...
Since chart transformations (by Definition \ref{definition:8-3}.(iii)) are smooth, we have that
Definition \ref{definition:8-7} is independent of the choice of charts in the given atlases for $M_1$
...
\begin{definition}\label{definition:8-8}
...
manifold again. A smooth atlas on N is given by all $h:U\cap N\to U'\cap\RR^k$, where
...
\begin{definition}\label{definition:8-10}
...
function theorem shows that it is a submanifold in the sense of Definition \ref{definition:8-8}.
...
\begin{align*}
  f(q) = (f_1(q), \ldots, f_d(q), \phi_1(q), \ldots, \phi_d(q) ).
\end{align*}
...
required by Definition \ref{definition:8-8}. The remaining $f(U_j)$ are treated analogously. Hence
...
\begin{remark}\label{remark:8-13}
  The general case of Theorem \ref{theorem:8-11} is shown in standard text books
...
Definition \ref{definition:8-8}, but with a homeomorphism instead of the diffeomorphism $h$.
\end{remark}
...
\begin{remark}\label{remark:8-16}
  ...
  Non-orientable surfaces: $\B{RP}^2$, Klein's bottle, etc. See e.g. [Hirsch]\supercite{Hirsch}, [Massey]\supercite{Massey}.
  ...
  topological 4-manifolds; see [Freedman-Quinn]\supercite{Freedman-Quinn}. At the same time S. Donaldson proved 
  ...
  new results on smooth 4-manifolds; see [Donaldson-Kronheimer]\supercite{Donaldson-Kronheimer}.
