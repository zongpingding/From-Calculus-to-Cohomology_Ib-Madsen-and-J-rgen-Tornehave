...
\begin{proof}
  We use Lemma \ref{lemma:A.9} to approximate $h$ by a smooth map $f:U\to V$. We
...
on $U\times(-\infty)\cup U\times(\frac23, \infty)$. Lemma \ref{lemma:A.9} allows us to approximate $H$ by a smooth map 
...
are chain-homotopic (see Definition \ref{definition:4-10}).
\end{theorem}
...
above we see that $f^*: H^p(V)\to H^p(U)$ is independent of the choice of $f$. Hence we can define
\begin{align*}
...
\begin{proof}
  The corollary follows from Theorem \ref{theorem:6-8}.(iii), as $h^{-1}:V\to U$ is a
...
\begin{proof}
  ...
  Then $U_1\cup U_2 = \RR_{n+1} - A$ and $U_1\cap U_2 = (\RR_n - A) x (-1,1)$. 
  Let $\phi: U_1\to U_1$ be given by adding 1 to the $(n + 1)$-st coordinate. For $x\in U_1$, $U_1$ contains the
...
\begin{addendum}\label{addendum:6-12}
  ...
  defined by $R(x_1, \ldots, x_n, x_{n+1}) = (x_1, \ldots, x_n, -x_{n+1})$. The induced linear map
  \begin{align*}
...
j_2^*(\omega_2)$. The definition of $\partial^*$ (see Definition \ref{definition:4-5}) show that $\partial^*([\omega]) = [\tau]$
...
$\R{diag} (1, \ldots, 1, \pm 1)$, where $d =\det A$. Hence it suffices to prove the assertion for
diagonal matrices. The matrices
\begin{align*}
  \R{diag}(1, \ldots, 1, \frac{|d|^td}{|d|}), \quad 0\le t\le 1
\end{align*}

yield a homotopy, which reduces the problem to the two cases $A = \R{diag} (1, \ldots, 1, \pm 1)$, so $f_A$ is 
