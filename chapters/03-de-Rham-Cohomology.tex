...
In this chapter $U$ will denote an open set in $\RR^n, {e_1,\ldots, e_n}$ the standard basis
and ${E_1, \ldots,E_n}$ the dual basis of $\alt^1(\RR^n)$.
...
In $\alt^p(\RR^n)$ we have the basis $\epsilon_1\wedge\ldots\wedge\epsilon_p$ where 
...
\begin{definition}\label{definition:3-2}
  The exterior differential\index{exterior differential}\index{differential!exterior} $\dd:\Omega^p(U)\to \Omega^{p+1}(U)$ is 
  the linear operator
  \begin{align*}
    \dd_x\omega(\xi_1,\ldots,\xi_{p+1})
    = \sum_{l=1}^{p+1}\left(-1\right)^{l-1}D_x\omega(\xi_l)(\xi_1,\ldots,\hat{\xi}_l,\ldots,\xi_{p+1})
  \end{align*}

  with $(\xi_1,\ldots,\hat{\xi}_l,\ldots,\xi_{p+1}) = (\xi_1, \ldots, \xi_{l-1}, \xi_{l+1}, \ldots, \xi_{p+1})$.
\end{definition}


It follows from Lemma \ref{lemma:2-7} that $\dd_x\omega\in\alt^{p+1}\RR^n$. Indeed, if $\xi_i = \xi_{i+1}$, then 
\begin{align*}
...
because $(\xi_1, \ldots, \hat{\xi}_i, \ldots, \xi_{p+1}) = (\xi_1, \ldots, \hat{\xi}_{i+1}, \ldots, \xi_{p+1})$.
...
\begin{example}\label{example:3-3}
  ...
  with $(\zeta^1, \ldots, \zeta^n) = \zeta$. In other words, $\dd f = \sum{\frac{\partial f}{\partial x_i}\dd x_i}$.  
\end{example}
...
\begin{proof}
  By \eqref{eq:3-1} we have
  \begin{align*}
    D_x\omega(\zeta)
    = (D_xf)(\zeta)\epsilon_I
    = \left(\frac{\partial f}{\partial x_1}\zeta^1+\cdots+\frac{\partial f}{\partial x_n}\zeta^n\right)\epsilon_I
    = \!\dd_xf(\zeta)\epsilon_I
  \end{align*}

  and Definition \ref{definition:3-2} gives
  \begin{align*}
    \dd_{x}\omega(\xi_{1},\ldots,\xi_{p+1}) 
    & = \sum_{k=1}^{p+1}\left(-1\right)^{k-1}\dd_{x}f(\xi_{k})\epsilon_{I}\left(\xi_{1},\ldots,\hat{\xi}_{k},\ldots,\xi_{p+1}\right) \\
    & = [\dd_{x}f\wedge\epsilon_{I}](\xi_{1},\ldots,\xi_{p+1}). 
  \end{align*}
\end{proof}
...
with $r$ the number determined by $i_r < k < i_{r+1}$ and $J=(i_1, \ldots, i_r, k, \ldots, i_p)$.
...
\begin{proof}
  ...
  \begin{align*}
    \dd(\omega_1\wedge\omega_2) 
    & = \dd(fg)\wedge\epsilon_{I}\wedge\epsilon_{J}=((\!\dd f)g+f\dd g)\wedge\epsilon_{I}\wedge\epsilon_{J} \\
    & = \dd fg\wedge\epsilon_{I}\wedge\epsilon_{J}+f\dd g\wedge\epsilon_{I}\wedge\epsilon_{J} \\
    & = \dd f\wedge\epsilon_{I}\wedge g\epsilon_{J}+(-1)^{p}f\epsilon_{I}\wedge \dd g\wedge\epsilon_{J} \\
    & = \dd\omega_{1}\wedge\omega_{2}+(-1)^{p}\omega_{1}\wedge \dd\omega_{2}. 
  \end{align*}
\end{proof}
...
There is precisely one linear operator $\dd:\Omega^0(U)\to\Omega^{p+1}(U), p = 0,1, \ldots,$ such that  
\begin{enumerate}[(i)]
...
\begin{proof}
We have already defined $\dd$ with the asserted properties. Conversely assume
...
constant map, and hence $\dd\epsilon_i=0$, by \eqref{eq:3-1}. Alternatively, we have already noted
...
\begin{definition}\label{definition:3-8}
The $p$-th (de Rham) cohomology group is the quotient vector space
\begin{align*}
  H^p(U) 
  = \frac {\ker\big(\!\dd\colon\Omega^p(U)\to\Omega^{p+1}(U)\big)}
          {\im(\!\dd\colon\Omega^{p-1}(U)\to\Omega^p(U))}.
\end{align*}
...
\begin{definition}\label{definition:3-10}
...
satisfied. Indeed,
\begin{align*}
  \phi^*(\omega)_x(\xi_1,\ldots,\xi_p)=\omega_{\phi(x)}(D_x\phi(\xi_1),\ldots,D_x\phi(\xi_p)),
\end{align*}
...
\begin{align*}
  ...
  & = \sum_{l=1}^{n}\frac{\partial\phi_{i}}{\partial x_{l}}\zeta^{l}
      =\sum_{l=1}^{n}\frac{\partial\phi_{i}}{\partial x_{l}}\epsilon_{l}(\zeta)=\dd\phi_{i}(\zeta).
\end{align*}

\begin{theorem}\label{theorem:3-12}
With Definition \ref{definition:3-10} we have the relations
\begin{enumerate}[(i)]
...
\begin{proof}
  Let $x\in U_1$ and let $\xi_1, \ldots, \xi_{p+q}$ be vectors in $\RR^n$. Then
  \begin{align*}
      & \phi^{*}(\omega\wedge\tau)_{x}(\xi_{1},\ldots,\xi_{p+q})
      = (\omega\wedge\tau)_{\phi(x)}(D_{x}\phi(\xi_{1}),\ldots,D_{x}\phi(\xi_{p+q})) \\
      & = \sum\mathrm{sign}(\sigma)\Big[\omega_{\phi(x)}\big(D_{x}\phi\big(\xi_{\sigma(1)}\big),\ldots,D_{x}\phi\big(\xi_{\sigma(p)}\big)\big) \\
      &\hspace*{5em} \tau_{\phi(x)}(D_x\phi(\xi_{\sigma(p+1)}),\ldots,D_x\phi(\xi_{\sigma(p+q)}))\Big] \\
      & = \sum\mathrm{sign}(\sigma)\phi^{*}(\omega)_{x}\big(\xi_{\sigma(1)},\ldots,\xi_{\sigma(p)}\big)
          \phi^{*}(\tau)_{x}\big(\xi_{\sigma(p+1)},\ldots,\xi_{\sigma(p+q)}\big) \\
      & = (\phi^{*}(\omega)_{x}\wedge\phi^{*}(\tau)_{x})(\xi_{1},\ldots,\xi_{p+q}).
  \end{align*}
...
\begin{align*}
  \dd\phi^{*}(\epsilon_{I})
  & = \dd\big(\phi^{*}(\epsilon_{i_{1}})\wedge\ldots\wedge\phi^{*}(\epsilon_{i_{p}})\big) \\
  & = \sum\left(-1\right)^{k-1}\phi^{*}(\epsilon_{i_{1}})\wedge\ldots\wedge\!\dd\phi^{*}(\epsilon_{i_{k}})\wedge\ldots\wedge\phi^{*}(\epsilon_{i_{p}})\\
  & = 0
\end{align*}  
...
\begin{align*}
  \dd x_I = \dd x_{i_1}\wedge\ldots\wedge\!\dd x_{i_p}
\end{align*}

instead of the (constant) $p$-form $\epsilon_{i_1}\wedge\ldots\wedge\epsilon_{i_p}$. An arbitrary 
...
\begin{enumerate}[(i)]
  \item Let $\gamma:(a, b)\to U$ be an smooth curve in $U$, $\gamma = (\gamma_1, \ldots, \gamma_n)$, and that 
    \begin{align*}
  ...
  \item Let $\phi:U_1\to U_2$ be a smooth map between open sets in $\RR^n$. Then
    \begin{align*}
      \phi^*(\!\dd x_1\wedge\ldots\wedge\dd x_n) = \det (D_x\phi)\dd x_1\wedge\ldots\wedge\dd x_n.
    \end{align*}
    indeed, from Theorem \ref{theorem:3-12},
    \begin{align*}
      \phi^{*}(\!\dd x_{1}\wedge\ldots\wedge \dd x_{n})
      & = \phi^*(\!\dd x_1)\wedge\ldots\wedge\phi^*(\!\dd x_n)
          = \dd\phi^*(x_1)\wedge\ldots\wedge \dd\phi^*(x_n)\\
      & = \dd\phi_1\wedge\ldots\wedge \dd\phi_n
          = \det{(D_{x}\phi)}\dd x_1\wedge\ldots\wedge \dd x_n.
    \end{align*}
...
where $I= (i_1, \ldots, i_p)$ and $J = (j_1, \ldots, j_{p-1})$. We define
\begin{align*}
...
\begin{align*}
  ...
  = \sum h_{I}(\psi(t)x)(d\psi(t)x_{i_{1}}+\psi(t)dx_{i_{1}})
    \wedge\ldots\wedge\left(d\psi(t)x_{i_{p}}+\psi(t)dx_{i_{p}}\right)  
\end{align*}
