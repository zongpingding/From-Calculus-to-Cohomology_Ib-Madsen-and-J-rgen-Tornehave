...
\begin{align*}
  \ker g & = \{b\in B \mid g(b) = 0\}\qquad \text{(the kernel of $g$)} \\
  \im f  & = \{f(a) \mid a\in A\}\qquad \text{(the image of $f$)}
\end{align*}
...
\begin{align*}
  \text{$f$ is injective,\quad $g$ is surjective\quad and\quad $\im f = \ker g$}
\end{align*}
...
\begin{proof}
  Let $\alpha\in A^p$ be a cycle ($\dd^pa = 0$) and $[a] = a + \im\dd^{p-1}$ its corresponding
