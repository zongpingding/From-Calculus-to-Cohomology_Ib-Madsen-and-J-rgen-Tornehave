...
\begin{proof}
  We write $N$ as a disjoint union of its connected components $N_1, \ldots, N_k$ and
...
The assertion then follows from diagram \eqref{eq:11-1}, since $H^n(f)$ depends only on the
homotopy class of $f$.
\end{proof}
...
\begin{proof}
  ...
  \begin{align}\label{eq:11-6}
    \R{diam}(T_l) = \frac{a\sqrt{n}}{N}\le \delta, \qquad T_l\cap K \neq 0\implies T_l\sseq L.
  \end{align}
  For a small cube $T_l$ with $T_l\cap K\neq \ns$ we pick $x\in T_l\cap K$. If $y\in T_l$ the 
  ...
  \begin{align}\label{eq:11-9}
    f(y) = f(x) + D_x f(y-x) + z,
  \end{align}
  where $z = (z_1, \ldots, z_n)$ is given by 
  \begin{align*}
...
\begin{lemma}\label{lemma:11-8}
  ...
  Then $f^{-1}(p)$ consists offinitely many points $q_1, \ldots, q_k$. Moreover, there exist 
...
consists of finitely many points $q_1, \ldots, q_k$. We can choose mutually disjoint open
...
Then $\supp_N(f^*(\omega))\sseq f^{-1}(U) = V_1\cup\ldots\cup V_k$, and we can write 
\begin{align*}
...
In the special case where $f^{-1}(p) = 0$ the theorem shows that $\R{deg}(f) = 0$ (in the
...
\begin{proposition}\label{proposition:11-11}
  ...
  $N_i^n,\ldots, N_k^n$. If $f_i = F_{|N_i}$, then
  \begin{align*}
...
\begin{definition}\label{definition:11-12}\index{linking number}
...
\begin{proposition}\label{prop:11-13} 
  \begin{enumerate}[(i)]
    ...
    \item Let $\Phi:P^{n+1}\to\RR^{n+1}-J$ be a smooth map with $P$ oriented. Given a compact domain 
      $R\sseq P$ with smooth boundary $\partial R$, let $Q_1, \ldots, Q_k$ be the connected components 
...
\begin{proof}
  ...
  \end{align*}
  Finally (iv) follows from Proposition \ref{proposition:11-11} applied to the map $F: J\times P\to S^n$ with
  \begin{align*}
...
\begin{align*}
  I(p) \{(q_1, q_2)\in J\times K\mid q_2-q_1 = \lambda p, \lambda>0\}.
\end{align*}
...
\begin{align*}
  \R{lk}(J, K_2) = \R{lk}(J, K_4)=0, \qquad \R{lk}(J, K_3) = 1,\qquad \R{lk}(J, K_1) = -1.
\end{align*}
\end{remark}
...
\begin{align*}
  \rho D^n = \{x\in\RR^n\mid \|x\|<\rho\}\sseq U
\end{align*}
...
\begin{definition}\label{definition:11-16}\index{degree}\index{local index}
  The degree of $F_\rho$ is called local index of $F$ at 0, and is denoted $\iota(F; 0)$.
\end{definition}
...
\begin{lemma}\label{lemma:11-17}
  ...
  \begin{align*}
    F:\RR^n - \{0\}\to \RR^n - \{0\}
  \end{align*}
  induces multiplication by $\iota(F; 0)$ on $H^{n-1}(\RR^n - \{0\})\simee \RR$.
\end{lemma}

\begin{proof}
  Let $i:S^{n-1}\to\RR^n-\{0\}$ be thw inclusion map and $r:\RR^{n-1}-\{0\}\to S^{n-1}$ the 
  restriction $r(x) = x/\|x\|$. We have $\iota(F; 0) = \R{deg}F_1$, where $F_1 = r\circ F\circ i$.
...
get $\iota((A^{-1})_*Y; 0) = \iota(Y; 0)$. Hence $\iota(F; 0) = \iota(Y; 0)$.
\end{proof}

\begin{definition}\label{definition:11-19}\index{local index}
  Let $X$ be a smooth tangent vector field on the manifold $M^n, n\ge 2$ with $p_0\in M$ as 
  an isolated zero. The local index $\iota(X; p_0)\in\ZZ$ of $X$ is defined by
  \begin{align*}
    \iota(X; p_0) = \iota(h_*X_{|U}; 0),
  \end{align*}
...
\begin{figure}[!htb]
  ...
  \caption{Figure 4(Left -- $\iota=-1$; Right -- $\iota=+2$)}
  \label{fig:11-4}
\end{figure}
...
\begin{lemma}\label{lemma:11-20}
  ...
  \begin{align*}
    \iota(X, p_0) = \R{sign}(\det D_0F)\in \{\pm 1\}.
  \end{align*}
\end{lemma}
...
between the map $F_\rho$ in Definition \ref{definition:11-16} and the analogous map $A_\rho$ with $A=D_0F$.
...
multiplication by $\iota(X; p_0)$; cf. Lemma \ref{lemma:11-17}. The result follows from 
Lemma \ref{lemma:6-14}.
\end{proof}

\begin{definition}\label{definition:11-21}
...
\begin{proof}
  Let $p_1, \ldots, p_k$ be the zeros in $R$ for $F$, and choose disjoint closed balls
$D_j\sseq R-\partial R$, with centers $p_j$. Define
\begin{align*}
  f_j: \partial D_j \to S^{n-1}; \qquad f_j(x) = F(x)/\|F(x)\|.
\end{align*}

We apply Proposition \ref{proposition:11-11} with $X = R-\cup_{j}\mrg{D}_j$. The boundary $\partial x$ 
is the disjoint union of aR and the $(n - 1)$-spheres $\partial D_1, \ldots, \partial D_k$. Here $\partial D_j$, 
...
Finally $\R{deg}(f_j) = \iota(F; p_j)$ by the definition of local index and Corollary \ref{corollary:11-3}.
\end{proof}
...
\begin{align}\label{eq:11-14}
  \iota(F; 0) = \sum_{p\in \tilde{F}^{-1}(p)}^{}{\iota(\tilde{F}, p)}
\end{align}
...
and it suffices to show that $\iota(X; p) = \iota(F; p)$ for an arbitrary zero of $X$. In 
...
where $f_i(0) = 0$, and by Lemma \ref{lemma:11-20} $\iota(X; p)$ is the sign of 
